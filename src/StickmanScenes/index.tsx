...
import { StickmanScenesLayer } from "./StickmanScenesLayer";
import { computeStoryBeats, RawBeat, storyBeats as fallbackStoryBeats, StoryBeat } from "./story";
...
// scenes/live_beats.json every run — an ordered RawBeat[] with a measured
// duration per clip. If it's not present (local dev / demo), we fall back to
// the static sample story in story.ts so the composition still renders.
const loadBeats = async (): Promise<StoryBeat[]> => {
  if (!hasLiveBeats()) return fallbackStoryBeats;
  const res = await fetch(staticFile(LIVE_BEATS_PATH));
  const raw = (await res.json()) as RawBeat[];
  return computeStoryBeats(raw);
};
...
export const StickmanStory: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "black" }}>
      <StickmanScenesLayer beats={beats} />
